...
void ClientStateDuckdb::setup_db_routines() {
  std::stringstream ss;
  ...
  {
    ss << "CREATE SCHEMA " << client_schema;
    db_exec(ss.str());
  }
  ...
  { // orders table
    ss << create_table << client_schema << ".orders ("
       ...
       << "basket_id INTEGER, "
       << "data BLOB, "
       << "FOREIGN KEY (basket_id) REFERENCES baskets(sid),"
       << "FOREIGN KEY (parent_order_id) REFERENCES orders(sid)"
       << ");";
  ...
  { // routes table
    ss << create_table << client_schema << ".routes ("
       ...
       << "data BLOB, "
       << "FOREIGN KEY (order_id) REFERENCES orders(sid)"
       << ")";
  ...
  { // fills table
    ss << create_table << client_schema << ".fills ("
       ...
       << "data BLOB, "
       << "FOREIGN KEY (order_id) REFERENCES orders(sid) , "
       << "FOREIGN KEY (route_id) REFERENCES routes(sid) , "
       << "FOREIGN KEY (original_id) REFERENCES fills(sid) "
       << ")";
  ...
  // Setup query statements
  {
    ...
    auto query = std::make_shared<duckdb::PreparedStatement>(*dbh, ss.str());
