#ifndef OMSCOMPARE_OMSMODEL_CLIENT_STATE_DUCKDB_H_
#define OMSCOMPARE_OMSMODEL_CLIENT_STATE_DUCKDB_H_
...
class ClientStateDuckdb : public ClientStateBase {
public:
  ClientStateDuckdb(types::ClientIdType client_id);
  ~ClientStateDuckdb();
...
private:
  ...
  std::shared_ptr<duckdb_database> db;
  std::shared_ptr<duckdb_connection> dbh;
  std::map<OperationId, std::shared_ptr<duckdb_prepared_statement>> statements;
